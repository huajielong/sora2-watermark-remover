...
from sora2wm.iopaint.const import MPS_UNSUPPORT_MODELS
        ...
        if model_md5:
            ...
            if model_md5 == _md5:
                ...
                try:
                    ...
                    logger.error(
                        f"Model md5: {_md5}, expected md5: {model_md5}, wrong model deleted. Please restart sora2wm.iopaint."
                        f"If you still have errors, please try download model manually first https://lama-Remover-docs.vercel.app/install/download_model_manually.\n"
                    )
                except:
                    logger.error(
                        f"Model md5: {_md5}, expected md5: {model_md5}, please delete {cached_file} and restart sora2wm.iopaint."
                    )
                exit(-1)
...
def handle_error(model_path, model_md5, e):
    ...
    if _md5 != model_md5:
        try:
            ...
            logger.error(
                f"Model md5: {_md5}, expected md5: {model_md5}, please delete {model_path} and restart sora2wm.iopaint."
            )
    else:
        logger.error(
            f"Failed to load model {model_path},"
            f"please submit an issue at https://github.com/Sanster/lama-Remover/issues and include a screenshot of the error:\n{e}"
        )
    exit(-1)
